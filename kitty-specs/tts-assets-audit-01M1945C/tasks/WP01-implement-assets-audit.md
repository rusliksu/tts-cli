...
- `16 passed`; `ruff check` и `ruff format --check` завершены успешно.
- Оба golden JSON прошли Draft 2020-12 schema validation.
- Privacy scan всего tracked tree не нашёл локальные пути, реальные Workshop ID
  или Steam asset URL.
- Три локальных сейва обработаны за `0.34`, `0.52` и `0.78` секунды;
  SHA-256 каждого входного файла до и после запуска совпал.
- Remote, release и публичная лицензия отсутствуют.
